...
def f_shiftadd0(seq, shift):
    if shift > 0:
        return np.concatenate((np.zeros([shift]),seq[:-shift]),axis=0)#数组拼接的函数
    else:
        return np.concatenate((seq[-shift:],np.zeros([-shift])),axis=0)

######## DNN player
#类似游戏的演进，从当前状态到下一个随机状态,模拟多次游戏，计算平均值
...
def updateDNN(memoryBuffer, lr):
	...
	######## with replacement
    # train DNN
    np.random.shuffle(memoryBuffer)#打乱数据的顺序，让训练数据集中的数据打乱顺序
...
#    DNN.loadParams('./bestParams/net_params.ckpt')
    DNN.saveParams('./bestParams/net_params.ckpt')#模型参数

    # performance of current DNN
    DNNplayer, DNNmin = DNN_play(n_games = 1, evaluating_fn = DNN.evaluate_node)  #n_games = 100
    meanCorr,_ = evaluate_DNN(n_games = args.eval_games, tau = 0, evaluating_fn = DNN.evaluate_node)
    ...
    episode = 0
    while episode < args.numEpisode:  # 'numEpisode': 100001,
        ...
        reward, correlation = calc_reward(cummulativeMove.reshape([1,len(cummulativeMove)]))#根据终端节点（游戏结果）序列来计算奖励值
