...
class Node():
	def __init__(self, selfPlay, cummulativeMove, num_Moves, n_actions, evaluating_fn, calc_reward, fromlink, parent=None):
		...
		self.move = cummulativeMove#序列
		self.num_Moves = num_Moves#格子总数
		self.n_actions = n_actions#2**l
	...
	def root_noise(self):
		self.Prior_sa = 0.75*self.reserve_Pr + 0.25*np.random.dirichlet(alpha0*np.ones(len(self.reserve_Pr[0])))#从迪利克雷函数中抽取样本
...
def MCTS_main(args, VisitedState, stepSize, n_steps, evaluating_fn, calc_reward, selfPlay):
    ...
    while eachstep < n_steps:
        ...
        for _ in range(args.simBudget):#模拟args.simBudget次游戏，不断的让树成型 。给当前节点一个反馈
            root.root_noise()#训练的时候加噪音诱发探索，避免每次走相同的路径
            v_l = TreePolicy(root, VisitedState, stepSize, flag)	# from v_0 to v_l   return node  画树
            Backup(v_l, v_l.value)	# back propagation      画树
        piVec = cal_piVec(root.N_sa, tau)

        # temporarily store
        currentState = np.append(cummulativeMove, np.zeros(args.N * args.K -len(cummulativeMove)))
        if selfPlay == 1:
            temp_store.append([currentState, piVec])#获取DNN训练的数据，当前节点状态和到达子节点的经验值

        # update state -> go to next time step
        nextMove = np.random.choice(args.Q ** stepSize, 1, p = piVec)[0]#根据pivec可能性分布，从0-2**l-1选择一个
    ...
    return cummulativeMove, temp_store #返回的是终端节点状态序列 和 祖先节点 和祖先节点的pivec
